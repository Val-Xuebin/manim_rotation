#!/usr/bin/env python3
# Compose: one image per task (question + answer + row Original|A|B|C|D). Optional text_output below. Out: composed_img/{id}_composed.jpg
import argparse
import json
import os
import re
...
def _resolve_img_path(batch_dir: Path, path_str: str) -> Path:
    """Resolve JSONL path to batch_dir/img/<basename>."""
    if "/img/" in path_str:
        name = path_str.split("/img/")[-1].strip()
    else:
        name = os.path.basename(path_str)
    return batch_dir / "img" / name
...
def _load_fonts(font_size: int):
    """Load Times New Roman (larger) or fallback serif."""
    small = max(12, font_size - 2)
    # Times New Roman common paths
    for path in [
...
def compose_one(
    ...
    """One canvas: text block + row of 5 images; labels centered below each image.
    If show_text_output is True, append task['text_output'] below the image row.
    """
    ...
    # Load images; scale to row_height, keep aspect ratio
    ...
    # Canvas width = full content width so text aligns to canvas
    row_width = sum(w for _, w, _ in imgs) + gap * 4
    canvas_w = max(max_text_width, row_width) + 2 * padding
    text_block_w = canvas_w - 2 * padding  # text fills canvas width
    ...
    # Optional: text_output block below image row
    ...
    # Text block: full width, left-aligned to canvas
    y = padding
    ...
    # Center image row; place each label centered under its image
    row_y = y
...
def main():
    parser = argparse.ArgumentParser(description="Compose task images from assign_data.jsonl into batch/composed_img/")
    ...
    parser.add_argument("--row-height", type=int, default=160, help="Image row height (aspect ratio preserved)")
    ...
    parser.add_argument("--font-size", type=int, default=22, help="Font size (default 22, Times New Roman when available)")
    parser.add_argument("--show-text-output", action="store_true", help="Draw task text_output below the image row")
